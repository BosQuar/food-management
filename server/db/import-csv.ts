...
if (!csvPath) {
  console.error("Usage: npx tsx server/db/import-csv.ts <path-to-csv>");
  process.exit(1);
}
...
for (const line of dataLines) {
  const [_add, name, category, notes] = parseCSVLine(line);
...
const categories: ParsedCategory[] = Array.from(categorySet.entries())
  .sort((a, b) => a[1] - b[1])
