...
import type { ParsedRecipe, ParsedIngredient } from "../../shared/types.js";
...
function extractJsonLd(html: string): JsonLdRecipe | null {
  ...
  while ((match = jsonLdRegex.exec(html)) !== null) {
    try {
      const data = JSON.parse(match[1]) as JsonLdRecipe | JsonLdRecipe[];
...
function findRecipeInJsonLd(
  data: JsonLdRecipe | JsonLdRecipe[],
): JsonLdRecipe | null {
...
function normalizeInstructions(
  instructions: string | JsonLdInstruction[] | undefined,
): string {
...
interface ParsedIngredientParts {
  amount: number | null;
  unit: string | null;
  name: string;
}
 ...
 */
function parseIngredientString(str: string): ParsedIngredientParts {
  str = str.trim();
