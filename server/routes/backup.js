...
router.get("/", (req, res) => {
  ...
  const backup = {
    ...
    data: {
      store_categories: db
        .prepare("SELECT * FROM store_categories ORDER BY sort_order")
        .all(),
      products: db.prepare("SELECT * FROM products ORDER BY id").all(),
      shopping_items: db
        .prepare("SELECT * FROM shopping_items ORDER BY id")
        .all(),
      recipes: db.prepare("SELECT * FROM recipes ORDER BY id").all(),
      recipe_ingredients: db
        .prepare("SELECT * FROM recipe_ingredients ORDER BY id")
        .all(),
    },
  };
  ...
  const transaction = db.transaction(() => {
    ...
    if (store_categories) {
      const insertCategory = db.prepare(
        "INSERT INTO store_categories (id, name, sort_order) VALUES (?, ?, ?)",
      );
    ...
    if (products) {
      const insertProduct = db.prepare(
        "INSERT INTO products (id, name, store_category_id, default_unit, default_notes, created_at) VALUES (?, ?, ?, ?, ?, ?)",
      );
    ...
    if (shopping_items) {
      const insertItem = db.prepare(
        "INSERT INTO shopping_items (id, product_id, custom_name, store_category_id, quantity, unit, notes, is_done, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      );
    ...
    if (recipes) {
      const insertRecipe = db.prepare(
        "INSERT INTO recipes (id, name, instructions, servings, source_url, created_at) VALUES (?, ?, ?, ?, ?, ?)",
      );
    ...
    if (recipe_ingredients) {
      const insertIngredient = db.prepare(
        "INSERT INTO recipe_ingredients (id, recipe_id, product_id, custom_name, amount, unit, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)",
      );
