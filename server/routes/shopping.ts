...
router.get("/", (req: Request, res: Response) => {
  ...
  const items = db
    ...
    .all(userId) as ShoppingItemWithDetails[];
...
router.post(
  "/",
  (req: Request<object, unknown, AddShoppingItemBody>, res: Response) => {
    const db = getDb();
    const userId = req.user!.id;
    ...
    const product = db
      ...
      .get(product_id, userId) as ProductRow | undefined;
    if (!product) {
	...
	`,
      ...
      .get(product_id, userId) as ShoppingItemRow | undefined;

    let item: ShoppingItemWithDetails;
    if (existing) {
      ...
      item = db
        ...
        .get(existing.id) as ShoppingItemWithDetails;
      ...
      item = db
        ...
        .get(result.lastInsertRowid) as ShoppingItemWithDetails;
...
router.post(
  "/custom",
  (req: Request<object, unknown, AddCustomItemBody>, res: Response) => {
    const db = getDb();
    const userId = req.user!.id;
	...
	`,
      ...
      .get(custom_name, effectiveUnit, userId) as ShoppingItemRow | undefined;

    let item: ShoppingItemWithDetails;
    if (existing) {
      // Update quantity
      const newQuantity = (existing.quantity || 0) + effectiveQuantity;
      db.prepare(
        `
			UPDATE shopping_items
...
router.put(
  "/:id",
  (
    req: Request<{ id: string }, unknown, UpdateShoppingItemBody>,
    res: Response,
  ) => {
    const db = getDb();
    const userId = req.user!.id;
    ...
    const existing = db
      ...
      .get(id, userId) as ShoppingItemRow | undefined;
    if (!existing) {
	...
	`,
      ...
      .get(id) as ShoppingItemWithDetails;
