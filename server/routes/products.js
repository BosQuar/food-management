...
router.post('/import', (req, res) => {
	...
	if (!categories || !products) {
		return res.status(400).json({ error: 'Missing categories or products in request body' });
	}
	...
	const transaction = db.transaction(() => {
		...
		for (const prod of products) {
			insertProduct.run(prod.id, prod.name, prod.store_category_id, prod.default_unit, prod.default_notes);
		}
	});

	transaction();

	res.json({ message: 'Import successful', categoriesImported: categories.length, productsImported: products.length });
});
...
router.post('/', (req, res) => {
	...
	const result = db.prepare(`
		...
		VALUES (?, ?, ?, ?)
	`).run(name, store_category_id || null, default_unit || 'st', default_notes || null);
