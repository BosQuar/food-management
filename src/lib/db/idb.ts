	...
	return new Promise((resolve, reject) => {
		...
		request.onupgradeneeded = (event) => {
			...
			if (!database.objectStoreNames.contains('products')) {
				...
				productsStore.createIndex('category', 'store_category_id', { unique: false });
			}

			// Shopping items store
			if (!database.objectStoreNames.contains('shopping')) {
				const shoppingStore = database.createObjectStore('shopping', { keyPath: 'id' });
				shoppingStore.createIndex('category', 'store_category_id', { unique: false });
			}
			...
			if (!database.objectStoreNames.contains('pending')) {
				const pendingStore = database.createObjectStore('pending', { keyPath: 'id', autoIncrement: true });
