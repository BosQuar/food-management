<script lang="ts">
  ...
  async function handleAddToShopping() {
    if (recipe) {
      const ingredientsToAdd = recipe.ingredients.filter((_, i) =>
        ...
        <AlertDialog.Content>
          <AlertDialog.Header>
            ...
            <AlertDialog.Description>
              Är du säker på att du vill ta bort "{recipe.name}"? Åtgärden kan
              inte ångras.
            </AlertDialog.Description>
          </AlertDialog.Header>
          <AlertDialog.Footer>
    ...
    {#if recipe.source_url}
      <p class="text-sm text-muted-foreground">
        Källa: <a
          ...
          target="_blank"
          rel="noopener"
          class="underline">{recipe.source_url}</a
        >
      </p>
    {/if}
  {/if}
...
<Dialog.Root bind:open={showAddToShoppingDialog}>
  <Dialog.Content class="max-w-md">
      ...
      {#if recipe}
        <div class="space-y-1">
          {#each recipe.ingredients as ingredient, index}
            <label
              class="flex items-center gap-3 cursor-pointer hover:bg-accent rounded-md p-2"
            >
              <Checkbox
    ...
    <Dialog.Footer>
      <Button
        variant="outline"
        onclick={() => (showAddToShoppingDialog = false)}>Avbryt</Button
      >
      <Button
